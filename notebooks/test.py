...
#     # ── Log Stage 2 FINAL decision into fairness monitor ──────────────────
#     # Merges demographic identity from stage1_customer (has gender/city_tier/rbi_consent)
#     # with bureau/financial fields from enhanced_customer_data.
#     # source='stage2' causes the helper to REPLACE the Stage 1 screening entry
#     # for the same application_id, so fairness dashboard always shows the binding decision.
#     try:
#         _s2_fairness_data = {**enhanced_customer_data, **{
#             k: stage1_customer.get(k)
#             for k in ('gender', 'city_tier', 'age', 'employment_type',
#                       'application_id', 'rbi_consent', 'loan_amount')
#             if stage1_customer.get(k) is not None
#         }}
#         log_decision_for_fairness(
#             customer_data   = _s2_fairness_data,
#             decision        = final_decision,
#             risk_score      = int(combined_risk_score or 0),
#             pd_pct          = float(stage2_result.get('pd_percentage')
#                                     or stage1_data.get('pd_percentage', 0) or 0),
#             application_id  = stage1_customer.get('application_id', ''),
#             source          = 'stage2',
#         )
#     except Exception:
#         pass  # Never block the UI for a logging side-effect
...
def display_stage2_results(stage2_result, stage1_data, stage1_customer, enhanced_customer_data):
    ...
    with tab4:
        ...
        # ── Regenerate reason codes using FINAL Stage 2 decision ──────────
        # Stage 1 stored REVIEW reasons — must reflect the Stage 2 APPROVE/REJECT outcome
        final_reasons = generate_reason_codes(
            decision           = final_decision,
            customer_data      = stage1_customer,
            affordability_data = stage1_data.get('affordability_data', {}),
            policy_checks      = stage1_data.get('policy_checks', {}),
        ...
        # ── Use Stage 2 values where available, fall back to Stage 1 ──────
        final_risk_score = int(combined_risk_score if combined_risk_score not in (None, 0) else stage1_data.get('risk_score', 0))
        final_pd         = float(stage2_result.get('pd_percentage') or stage1_data.get('pd_percentage', 0))
        final_conf       = float(stage2_confidence or confidence)

        report_data = {
            'application_id': _safe(stage1_customer.get('application_id'), 'N/A'),
            'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            'decision': final_decision,           # Stage 2 final (not Stage 1 REVIEW)
            'risk_score': final_risk_score,        # Stage 2 combined score
            'pd_percentage': final_pd,             # Stage 2 PD
            'confidence': final_conf,              # Stage 2 confidence
            'policy_checks': stage1_data.get('policy_checks', {}),
            'affordability_data': stage1_data.get('affordability_data', {}),
            'customer_data': stage1_customer,
            'reason_codes': final_reasons,         # Regenerated for APPROVE/REJECT
            'pd_calculation_factors': {
                'bureau_score': bureau_score,
                'base_pd': bureau_score_to_pd(bureau_score),
                'dpd_90': dpd_90, 'dpd_30': dpd_30,
                ...
                'foir_adjustment': foir_to_pd_adjustment(foir),
                'employment_adjustment': employment_stability_to_pd_adjustment(employment_type, employment_tenure, business_vintage),
                'ml_adjustment': ml_confidence_to_pd_adjustment(final_conf, final_decision),
                'final_pd': final_pd
            },
            'stage2_final_decision': _safe(final_decision, 'N/A'),
            'stage2_tier': _safe(stage2_tier, 'N/A'),
