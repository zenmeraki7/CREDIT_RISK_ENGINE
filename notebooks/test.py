...
def resolve_stage2_to_binary(stage2_result: dict) -> dict:
    """
    Normalise Stage 2 result to a binary APPROVE / REJECT / REVIEW decision.
    - REJECT stays REJECT
    - APPROVE mapped through tier (P1/P2 → APPROVE, P3/P4 → REJECT)
    - REVIEW: if Stage 2 ran and has a tier → map via tier
              if Stage 2 failed (stage2_used=False) → keep as REVIEW
                (the engine already set a clear error message as the reason)
              if no tier and Stage 2 ran → use score (0-1000 scale) to decide
    """
    result = stage2_result.copy()
    tier       = result.get('stage2_tier', '') or ''
    raw        = result.get('final_decision', '')
    score      = result.get('combined_risk_score', 0) or 0
    s2_ran     = result.get('stage2_used', True)
    ...
    if raw == 'REJECT':
        ...
        if not s2_ran:
            # Stage 2 model unavailable — keep as REVIEW, do not auto-REJECT
            result['final_decision'] = 'REVIEW'
            result['interest_rate_range'] = 'N/A — Requires Manual Review'
            ...
            # Stage 2 ran but no tier — use 0-1000 combined score
            resolved = 'APPROVE' if score >= 600 else 'REJECT'
            result['final_decision'] = resolved
            result['reason'] = result.get('reason', '') + f" [REVIEW resolved to {resolved} via score {score}/1000]"
    else:
        result['final_decision'] = raw or 'REVIEW'

    if result['final_decision'] == 'APPROVE':
        result.setdefault('interest_rate_range', {'P1': '9.5%–11%', 'P2': '11%–13%'}.get(tier, '11%–14%'))
    elif result['final_decision'] == 'REVIEW':
        result.setdefault('interest_rate_range', 'N/A — Requires Manual Review')
    else:
...
def display_stage2_results(stage2_result, stage1_data, stage1_customer, enhanced_customer_data):
    ...
    with tab1:
        ...
        # FIX M3: Show tier decision prefix as a clean info banner,
        # then render ALL stage2_reason_codes as a numbered list.
        # Previously st.info(reason) showed one cramped run-on string
        # and the stage2_reason_codes list was never displayed.
        _tier_prefix = stage2_result.get('reason', '')
        if _tier_prefix:
            st.info(_tier_prefix)
        s2_reason_codes = stage2_result.get('stage2_reason_codes', [])
        if s2_reason_codes:
            st.markdown('**📋 Detailed Decision Reasons:**')
            for _idx, _reason in enumerate(s2_reason_codes, 1):
                st.markdown(f'<div style="padding:0.3rem 0.5rem;border-left:3px solid #587042;margin:0.2rem 0;"><span style="font-weight:600;color:#587042;">{_idx}.</span> {_reason}</div>', unsafe_allow_html=True)
        else:
            st.caption("ℹ️ No detailed reason codes available for this decision.")
        ...
        if PDF_AVAILABLE and generate_audit_pdf is not None:
            try:
                ...
                _final_pd  = stage1_data.get('pd_percentage', round(max(0.5, min(
                    _base_pd * _deliq_mul + _foir_adj + _emp_adj + _inq_adj + _ml_adj, 50.0)), 2))

                # FIX M4: Merge Stage 1 + Stage 2 reason codes for the PDF.
                # Previously only Stage 1 reasons appeared in the audit report —
                # Stage 2 reasons (tier quality, DPD, utilization, etc.) were absent.
                _s1_reasons = stage1_customer.get('reason_codes', [])
                _s2_reasons = stage2_result.get('stage2_reason_codes', [])
                _final_dec  = stage2_result.get('final_decision', '')
                # FIX: don't include Stage 1 REVIEW reasons when Stage 2 APPROVEs.
                # Stage 1 REVIEW reasons (e.g. "EMI burden elevated") are based on
                # the Stage 1 decision, not the final binding decision. Including
                # them in an APPROVE report is misleading and confusing.
                # Only carry S1 reasons through when they agree with the outcome
                # (i.e. both are REJECT) or Stage 2 didn't run.
                if _final_dec == 'APPROVE':
                    _all_reasons = [f"[Stage 2] {r}" for r in _s2_reasons] if _s2_reasons else _s1_reasons
                else:
                    _all_reasons = _s1_reasons + (
                    [f"[Stage 2] {r}" for r in _s2_reasons] if _s2_reasons else []
                )
                ...
                report_data = {
                    ...
                    'risk_score':                 _safe(combined_risk, 0),   # combined 0-1000
                    'stage1_risk_score':          _safe(stage1_data.get('risk_score', 0), 0),  # S1 0-100
                    ...
                    # Reason codes — Stage 1 + Stage 2 merged (FIX M4)
                    'reason_codes': _all_reasons,
                    # Raw data refs
    ...
    with col1:
        if st.button("🔄 New Assessment", key="new_assessment_stage2", width='stretch'):
            ...
            # Clear all OCR/pdf pre-fill fields so they don't bleed into the next applicant
            _pdf_keys = [k for k in st.session_state if k.startswith('pdf_')]
            for k in _pdf_keys:
                del st.session_state[k]
            st.session_state.page_navigation = "👤 Assessment"
            st.rerun()
    with col2:
        if st.button("← Back to Stage 1", key="back_to_stage1", width='stretch'):
            st.session_state.page_navigation = "👤 Assessment"
            st.rerun()
    with col3:
        if st.button("🏠 Home", key="home_stage2", width='stretch'):
...
with st.sidebar:
    ...
    if st.session_state.stage1_complete:
        ...
        if st.button("🔄 New Assessment", key="new_assessment_sidebar", width='stretch'):
            for k in ['stage1_complete','stage1_decision','stage1_data','current_customer_data','extracted_cibil_data']:
                st.session_state[k] = False if k == 'stage1_complete' else None
            # Clear all OCR/pdf pre-fill fields
            _pdf_keys = [k for k in st.session_state if k.startswith('pdf_')]
            for k in _pdf_keys:
                del st.session_state[k]
            st.rerun()
...
if page == "🏠 Home":
        ...
        if pdf_just_extracted:
            ...
            if not OCR_AVAILABLE:
                ...
                if uploaded_pdf is not None:
                    ...
                    if st.button("🔍 Extract & Auto-fill Form", key="extract_assessment", type="primary", width='stretch'):
                        ...
                        if extraction_result.get('success', False):
                            st.session_state.pdf_age               = min(int(extraction_result.get('AGE', 35)), 70)  # form max=70
                            st.session_state.pdf_bureau_score      = max(300, min(int(extraction_result.get('Credit_Score', 720)), 900))  # form range 300-900
    ...
    with st.form("assessment_form"):
        ...
        with col1:
            ...
            dpd_90_6m = st.number_input("DPD 90+ (Last 6M)", 0, 20,
                value=min(int(st.session_state.get('pdf_dpd_90', 0)), 20))
            dpd_30_6m = st.number_input("DPD 30+ (Last 6M)", 0, 20,
                value=min(int(st.session_state.get('pdf_dpd_30', 0)), 20))
        with col2:
            credit_utilization = st.number_input("Credit Utilization (%)", 0, 100,
                value=min(int(st.session_state.get('pdf_credit_util', 30)), 100))
            recent_inquiries = st.number_input("Recent Inquiries (3M)", 0, 20,
                value=min(int(st.session_state.get('pdf_inquiries', 2)), 20))
        with col3:
            _active_loans_max = 30
            active_loans = st.number_input("Active Loans", 0, _active_loans_max,
                value=min(int(st.session_state.get('pdf_active_loans', 1)), _active_loans_max))
    ...
    if submitted:
        ...
        with tab2:
            ...
            if final_decision in ['APPROVE', 'REVIEW']:
                ...
                col1, col2, col3 = st.columns(3)
                with col1:
                    if st.button("📝 Manual Entry", key="stage2_manual_btn", width='stretch', type="primary"):
                        st.session_state.stage2_selected_tab = "Manual Entry"
                        st.session_state.page_navigation = "🔬 Stage 2 Analysis"
                        st.rerun()
                with col2:
                    if st.button("📄 PDF Upload", key="stage2_pdf_btn", width='stretch', type="primary"):
                        st.session_state.stage2_selected_tab = "PDF Upload"
                        st.session_state.page_navigation = "🔬 Stage 2 Analysis"
                        st.rerun()
                with col3:
                    if st.button("📊 Batch Analysis", key="stage2_batch_btn", width='stretch', type="primary"):
            ...
            with col2:
                if st.button("🔄 Re-Evaluate", key="reevaluate_btn", width='stretch'):
                    st.rerun()
        ...
        with tab3:
            ...
            col1, col2 = st.columns(2)
            with col1:
                st.plotly_chart(create_modern_gauge(decision_data.get('confidence', 0), "Model Confidence"), width='stretch')
            with col2:
                st.plotly_chart(create_modern_bar_chart(decision_data.get('class_probs', {"APPROVE": 0, "REVIEW": 0, "REJECT": 0})), width='stretch')
    ...
    if selected_tab == "Manual Entry":
        ...
        with st.form("stage2_manual_form"):
            ...
            st.markdown("<br>", unsafe_allow_html=True)
            submitted_s2 = st.form_submit_button("🔬 Run Stage 2 Analysis", width='stretch', type="primary")
        ...
        if not OCR_AVAILABLE:
            ...
            if uploaded_pdf is not None:
                ...
                if st.button("🔬 Extract & Analyze", key="extract_analyze_stage2", type="primary", width='stretch'):
                    ...
                    if extraction_result.get('success', False):
                        ...
                        # Identity validation — warn if CIBIL PDF doesn't match Stage 1 applicant
                        _id_warnings = validate_cibil_identity(stage1_customer, extraction_result)
                        if _id_warnings:
                            for _w in _id_warnings:
                                st.warning(f"⚠️ Identity check: {_w}")
    ...
    if uploaded_file is not None:
        try:
            ...
            if missing_cols:
                ...
                if st.button("🚀 Process Batch Predictions", key="process_batch_btn", type="primary", width='stretch'):
