...
from risk_engine import (
    ...
    fill_missing_ml_fields,
    clean_sentinel_values,
    validate_cibil_identity,
)
...
# =============================================================================
# PD CALCULATION FUNCTIONS (unchanged)
# =============================================================================
def bureau_score_to_pd(bureau_score):
    if bureau_score >= 800:
        return 0.5 + (900 - bureau_score) / 200 * 0.5
...
# =============================================================================
# AFFORDABILITY CALCULATION ENGINE (using imported functions)
# =============================================================================
def calculate_emi(principal, annual_rate, tenure_months):
    if principal <= 0 or tenure_months <= 0:
        return 0
...
# =============================================================================
# REASON CODE GENERATION SYSTEM (using imported function)
# =============================================================================
...
def extract_cibil_from_pdf(uploaded_file):
    ...
    try:
        ...
        # ----- Extraction logic (unchanged, but we add business_vintage) -----
        credit_score = 720
        score_match = re.search(r'\b(\d{3})\s*(?:EXCELLENT|VERY\s*GOOD|GOOD|FAIR|SUBPRIME|POOR|NH|NA)\b', full_text, re.IGNORECASE)
        if score_match:
            val = int(score_match.group(1))
            if 300 <= val <= 900:
                credit_score = val
        if credit_score == 720:
            score_match2 = re.search(r'(?:cibil|credit)\s*score\s*[:\-\(]?\s*(\d{3})', full_text, re.IGNORECASE)
            if score_match2:
                val = int(score_match2.group(1))
                if 300 <= val <= 900:
                    credit_score = val
        ...
        monthly_income = 50000
        income_match = re.search(r'(?:net\s+monthly\s+income|monthly\s+income|net\s+income|salary)[^\n\r]{0,30}?'
                                 r'(?:rs\.?\s*|inr\s*|₹\s*)([\d,]+)', full_text, re.IGNORECASE)
        if income_match:
            val = int(income_match.group(1).replace(',', ''))
            if val > 1000:
                monthly_income = val
        ...
        age_extracted = 35
        dob_match = re.search(r'(?:date\s+of\s+birth|dob)[:\s]+(\d{2}[-/]\w{3,9}[-/]\d{2,4})', full_text, re.IGNORECASE)
        if dob_match:
            try:
                from datetime import datetime as _dt
                dob_str = dob_match.group(1)
        ...
        for line in lines:
            ...
            if in_accounts:
                ...
                if not stripped:
                    continue
                dpd_match = re.search(r'\b(0\d0|0\d\d|\d{3})\b', stripped)
                status_match = re.search(r'\b(Active|Settled|Written[-\s]?Off|Closed|NPA|Doubtful|Loss)\s*$', stripped, re.IGNORECASE)
                if (re.search(r'\bINR\b', stripped, re.IGNORECASE) or
                        re.match(r'^[A-Z][a-zA-Z\s]+(?:Bank|Finance|Capital|Fincorp|SBI|ICICI|HDFC|Axis|Bajaj|Tata|Kotak)', stripped)):
                    dpd_val = int(dpd_match.group(1)) if dpd_match else 0
                    status_str = status_match.group(1) if status_match else 'Active'
        ...
        if active_count == 0:
            summary_match = re.search(r'Total\s+Accounts\s+Active[^\n]*\n\s*(\d+)\s+(\d+)', full_text, re.IGNORECASE)
            if summary_match:
                active_count = int(summary_match.group(2))
            else:
                inline = re.search(r'(?:Total\s+Accounts.*?Active.*?Closed.*?\n|'
                                   r'(\d+)\s+(\d+)\s+(\d+)\s+[\d,]+\s+\d+)', full_text, re.IGNORECASE)
                if inline and inline.group(2):
                    active_count = int(inline.group(2))
        ...
        extracted_data = {
            ...
            'AGE': age_extracted,
            'NETMONTHLYINCOME': monthly_income,
            'Time_With_Curr_Empr': biz_vintage * 12,   # months
            'business_vintage': biz_vintage,            # years (FIXED: added)
            'CC_Flag': 1 if re.search(r'credit card', full_text, re.IGNORECASE) else 0,
            'PL_Flag': 1 if re.search(r'personal loan', full_text, re.IGNORECASE) else 0,
            'HL_Flag': 1 if re.search(r'home loan', full_text, re.IGNORECASE) else 0,
            'GL_Flag': 1 if re.search(r'gold loan', full_text, re.IGNORECASE) else 0,
...
# =============================================================================
# HYBRID DECISION ENGINE – FULLY CORRECTED
# =============================================================================
def make_hybrid_decision_enhanced(customer_dict):
    # ---- Helper for policy reject ----
    def _policy_reject(reason, policy_checks):
        return {
            'decision': "REJECT",
            'reason': f"Policy Gate: {reason}",
            'confidence': 0,
            'class_probs': {'REJECT': 100},
            'policy_checks': policy_checks,
            'risk_score': 0,
            'pd_percentage': 25.0,          # capped at max of scale
            'affordability_data': {}
        }

    policy_checks = {}

    # ---- Fill missing ML fields (critical) ----
    ...
    if age < age_min or age > age_max:
        policy_checks['age'] = f"❌ Age {age} (Required: {age_min}-{age_max})"
        return _policy_reject("Age outside allowed range", policy_checks)
    policy_checks['age'] = f"✅ Age {age} (Valid)"

    if not kyc_verified:
        policy_checks['kyc'] = "❌ KYC Not Verified"
        return _policy_reject("KYC verification required", policy_checks)
    policy_checks['kyc'] = "✅ KYC Verified"

    if bankruptcy_flag:
        policy_checks['bankruptcy'] = "❌ Active Bankruptcy"
        return _policy_reject("Active bankruptcy", policy_checks)
    policy_checks['bankruptcy'] = "✅ No Bankruptcy"

    if fraud_flag:
        policy_checks['fraud'] = "❌ Fraud Flag"
        return _policy_reject("Fraud detected", policy_checks)
    policy_checks['fraud'] = "✅ No Fraud History"
    ...
    if monthly_income < 15000:
        ...
        return _policy_reject("Income below minimum", policy_checks)
    policy_checks['income'] = f"✅ Income ₹{monthly_income:,.0f}"

    # Tenure
    if employment_type == 'Salaried' and employment_tenure < 6:
        policy_checks['tenure'] = f"❌ Tenure {employment_tenure} months (Min: 6)"
        return _policy_reject("Insufficient tenure", policy_checks)
    elif employment_type in ['Self-Employed', 'Business'] and business_vintage < 2:
        policy_checks['tenure'] = f"❌ Business Vintage {business_vintage} years (Min: 2)"
        return _policy_reject("Insufficient business vintage", policy_checks)
    policy_checks['tenure'] = (
        f"✅ Tenure {employment_tenure} months" if employment_type == 'Salaried'
    ...
    if bureau_score < 550:
        policy_checks['bureau'] = f"❌ Bureau Score {bureau_score} (Min: 550)"
        return _policy_reject("Bureau score too low", policy_checks)
    policy_checks['bureau'] = f"✅ Bureau Score {bureau_score}"

    # DPD 90+
    if dpd_90 > 0:
        policy_checks['dpd'] = f"❌ {dpd_90} instances of 90+ DPD"
        return _policy_reject("Severe delinquency", policy_checks)
    policy_checks['dpd'] = "✅ No 90+ DPD"

    # Other credit indicators (soft)
    credit_utilization = customer_dict.get('credit_utilization_pct', 0)
    recent_inquiries = customer_dict.get('recent_inquiries_3m', 0)
    active_loans = customer_dict.get('active_loans_count', 0)
    salary_stability = customer_dict.get('salary_stability_flag', 'STABLE')
    net_surplus = customer_dict.get('net_cash_surplus_6m', 0)
    bounces = customer_dict.get('inward_bounce_count_3m', 0)  # filled by fill_missing_ml_fields
    ...
    interest_rate = customer_dict.get('interest_rate', 10.5)
    existing_emi = customer_dict.get('existing_emi', 0)
    affordability_data = calculate_affordability(monthly_income, loan_amount, interest_rate,
                                                 loan_tenure, existing_emi)
    foir = affordability_data['foir_percentage']
    net_disposable = affordability_data['net_disposable']

    # ---- Additional checks ----
    # Loan-to-income
    annual_income = monthly_income * 12
    lti = check_loan_to_income(loan_amount, annual_income)
    if lti['status'] in ('high', 'extreme'):
        policy_checks['loan_income'] = f"⚠️ Loan-to-income ratio {lti['ratio']}x"
        if ml_decision == 'APPROVE' and lti['status'] == 'extreme':
            ml_decision = 'REVIEW'
    else:
        policy_checks['loan_income'] = f"✅ Loan-to-income ratio {lti['ratio']}x"

    # Net disposable
    nd = check_net_disposable(net_disposable)
    if nd['status'] in ('low', 'critical'):
        policy_checks['net_disposable'] = f"⚠️ Net disposable ₹{net_disposable:,} (min ₹10,000)"
        if ml_decision == 'APPROVE' and nd['status'] == 'critical':
            ml_decision = 'REVIEW'
    else:
        policy_checks['net_disposable'] = f"✅ Net disposable ₹{net_disposable:,}"

    # Active loans
    if active_loans >= 5:
        policy_checks['active_loans'] = f"⚠️ High active loans ({active_loans}) – Review"
        if ml_decision == 'APPROVE':
            ml_decision = 'REVIEW'
    else:
        policy_checks['active_loans'] = f"✅ Active loans: {active_loans}"
...
# =============================================================================
# MODERN UI COMPONENTS (now using decision_summary.py)
# =============================================================================
...
# =============================================================================
# STAGE 2 RESULTS DISPLAY FUNCTION (with identity validation and sentinel cleaning)
# =============================================================================
def display_stage2_results(stage2_result, stage1_data, stage1_customer, enhanced_customer_data):
    """Display comprehensive Stage 2 results with decision report and download options"""
...
# =============================================================================
# ASSESSMENT PAGE (with new behavioral fields)
# =============================================================================
elif page == "👤 Assessment":
    st.markdown('<p class="main-header">Credit Assessment</p>', unsafe_allow_html=True)
            ...
            if uploaded_pdf is not None:
                ...
                if st.button("🔍 Extract & Auto-fill Form", key="extract_assessment", type="primary",
                    ...
                    with st.spinner("🔄 Running OCR on CIBIL PDF — this takes 10-30 seconds..."):
                        extraction_result = extract_cibil_from_pdf(uploaded_pdf)
                    if extraction_result.get('success', False):
                        # Store ALL extracted values into session_state
                        st.session_state.pdf_age              = int(extraction_result.get('AGE', 35))
                        st.session_state.pdf_employment_type  = 'Salaried'
                        st.session_state.pdf_kyc              = True
                        st.session_state.pdf_bankruptcy       = False
    ...
    with st.form("assessment_form"):
        ...
        # ===== NEW BEHAVIORAL INDICATORS SECTION =====
        st.markdown('<p class="section-header">📉 Behavioral Indicators</p>', unsafe_allow_html=True)
        col1, col2, col3 = st.columns(3)
        with col1:
            payment_discipline = st.selectbox("Payment Discipline", ['GOOD', 'MODERATE', 'POOR'], index=0)
            liquidity_flag = st.selectbox("Liquidity", ['ADEQUATE', 'LOW'], index=0)
        with col2:
            cashflow_health = st.selectbox("Cashflow Health", ['HEALTHY', 'MODERATE', 'STRESSED'], index=1)
            bureau_risk_flag = st.selectbox("Bureau Risk", ['LOW', 'MEDIUM', 'HIGH'], index=0)
        with col3:
            bounce_count = st.number_input("Bounce Count (3M)", 0, 10, 0)
            salary_missing_months = st.number_input("Missing Salary Months", 0, 6, 0)
        col1, col2, col3 = st.columns(3)
        with col1:
            total_late_payments_6m = st.number_input("Late Payments (30+ days, 6M)", 0, 50, 0)
        with col2:
            total_late_payments_90_6m = st.number_input("Late Payments (90+ days, 6M)", 0, 50, 0)
    ...
    if submitted:
        ...
        customer_data = {
            ...
            'total_late_30_6m': total_late_payments_6m,
            'total_late_90_6m': total_late_payments_90_6m,
        }
        ...
        with tab3:
            ...
            with col1:
                ...
                st.plotly_chart(fig1, use_container_width=True)
            with col2:
                # FIXED: always use real class_probs
                class_probs = decision_data.get('class_probs', {"APPROVE": 0, "REVIEW": 0, "REJECT": 0})
                # Show override info if needed
                if decision_data.get('ml_raw_decision') and decision_data.get('decision') != decision_data.get('ml_raw_decision'):
                    st.info(f"ℹ️ ML predicted {decision_data['ml_raw_decision']} – overridden to {decision_data['decision']} by policy rules.")
                fig2 = create_modern_bar_chart(class_probs)
                st.plotly_chart(fig2, use_container_width=True)
...
# =============================================================================
    ...
    # =========================================================================
    # TAB 2: PDF UPLOAD (with identity validation and income guard)
    # =========================================================================
    elif selected_tab == "PDF Upload":
        st.markdown('<p class="section-header">📄 CIBIL PDF Upload</p>', unsafe_allow_html=True)
        if not OCR_AVAILABLE:
            ...
            if uploaded_pdf is not None:
                ...
                if st.button("🔬 Extract & Analyze", key="extract_analyze_stage2", type="primary", use_container_width=True):
                    with st.spinner("🔄 Extracting data from PDF..."):
                        ...
                        if extraction_result.get('success', False):
                            # ===== IDENTITY VALIDATION =====
                            warnings = validate_cibil_identity(stage1_customer, extraction_result)
                            if warnings:
                                for w in warnings:
                                    st.warning(f"⚠️ {w}")
                                confirm = st.checkbox("I confirm this CIBIL report belongs to the same applicant")
                                if not confirm:
                                    st.stop()

                            # ===== CLEAN SENTINEL VALUES =====
                            extraction_result = clean_sentinel_values(extraction_result)

                            # ===== INCOME GUARD =====
                            s1_income = stage1_customer.get('avg_salary_6m', 0)
                            s2_income = extraction_result.get('NETMONTHLYINCOME', 0)
                            if s2_income > 0 and s1_income > 0 and (s1_income / s2_income) > 2:
                                st.warning(f"Income differs: Application ₹{s1_income:,} vs CIBIL ₹{s2_income:,}. Using application income.")
                                extraction_result['NETMONTHLYINCOME'] = s1_income
    ...
    # =========================================================================
    # TAB 3: BATCH ANALYSIS (improved placeholder)
    # =========================================================================
    elif selected_tab == "Batch Analysis":
        st.markdown('<p class="section-header">📊 Batch CIBIL Analysis</p>', unsafe_allow_html=True)
        st.info("📊 Batch analysis feature coming in v9.0 – you will be able to upload a CSV with CIBIL fields for bulk Stage 2 processing.")
        st.markdown("""
        **Required columns would include:**
        `Credit_Score`, `num_times_30p_dpd`, `num_times_60p_dpd`, `enq_L3m`, `CC_utilization`, `NETMONTHLYINCOME`, etc.
        """)
...
# =============================================================================
    ...
    if uploaded_file is not None:
        ...
        template_data = {
            ...
            'existing_emi': [15000, 20000, 8000],
            'AMT_ANNUITY': [8500, 9500, 4500],
            # New fields (optional in template)
            'payment_discipline_flag': ['GOOD', 'MODERATE', 'POOR'],
            'liquidity_flag': ['ADEQUATE', 'ADEQUATE', 'LOW'],
            'cashflow_health': ['HEALTHY', 'MODERATE', 'STRESSED'],
            'bureau_risk_flag': ['LOW', 'MEDIUM', 'HIGH'],
            'inward_bounce_count_3m': [0, 1, 2],
            'salary_missing_months': [0, 0, 1],
            'total_late_30_6m': [0, 2, 3],
            'total_late_90_6m': [0, 1, 1],
        }
        template_df = pd.DataFrame(template_data)
        st.dataframe(template_df, use_container_width=True)
        csv_template = template_df.to_csv(index=False)
