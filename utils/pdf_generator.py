...
def generate_decision_pdf(decision_data, customer_data, affordability_data, reasons):
    ...
    header_data = [
        ['Application ID:', app_id, 'Timestamp:', timestamp],
        ['Decision:', decision, 'Risk Score:', f"{decision_data.get('risk_score', 0)}/100"],
        ['PD Score:', f"{decision_data.get('pd_percentage', 0)}%", 'Confidence:', f"{decision_data.get('confidence', 0):.1f}%"]
    ]
    ...
    # Customer Information
    story.append(Paragraph("CUSTOMER INFORMATION", heading_style))
    rbi_consent_text = 'Obtained ✓' if customer_data.get('rbi_consent', False) else 'NOT OBTAINED ✗'
    customer_info_data = [
        ['Name:', customer_data.get('name', 'N/A'), 'Gender:', customer_data.get('gender', 'N/A')],
        ['Age:', str(customer_data.get('age', 'N/A')), 'Employment:', customer_data.get('employment_type', 'N/A')],
        ...
        ['Interest Rate:', f"{customer_data.get('interest_rate', 0)}%", 'KYC Status:', 'Verified' if customer_data.get('kyc_verified', True) else 'Not Verified'],
        ['City Tier:', customer_data.get('city_tier', 'N/A'), 'RBI Consent:', rbi_consent_text],
    ]
    ...
    story.append(Paragraph("RISK ASSESSMENT", heading_style))
    risk_data = [
        ['Risk Score:', f"{decision_data.get('risk_score', 0)}/100 (higher = riskier)"],
        ['PD (Probability of Default):', f"{decision_data.get('pd_percentage', 0)}%"],
        ['Model Confidence:', f"{decision_data.get('confidence', 0):.1f}%"],
        ['Bureau Score:', str(customer_data.get('bureau_score', 0))],
        ['DPD 90+ (6M):', str(customer_data.get('dpd_90_count_6m', 0))],
    ...
    # ── KEY FACT STATEMENT (KFS) — RBI Digital Lending Guidelines 2022 ──────
    story.append(Paragraph("KEY FACT STATEMENT (KFS)", heading_style))
    loan_amount  = customer_data.get('loan_amount', 0)
    interest_rate = customer_data.get('interest_rate', 0)
    loan_tenure  = customer_data.get('loan_tenure_months', 0)
    new_emi      = decision_data.get('affordability_data', {}).get('new_emi', 0)
    apr_approx   = round(interest_rate * 1.02, 2)  # indicative APR incl. processing fee estimate
    kfs_data = [
        ['Sanctioned Loan Amount:', f"Rs.{loan_amount:,}", 'Loan Tenure:', f"{loan_tenure} months"],
        ['Annual Interest Rate:', f"{interest_rate}% p.a.", 'Approx. APR (incl. fees):', f"~{apr_approx}% p.a."],
        ['Monthly EMI:', f"Rs.{new_emi:,}", 'Total Interest Payable:', f"Rs.{max(0, int(new_emi * loan_tenure - loan_amount)):,}"],
        ['Total Amount Payable:', f"Rs.{int(new_emi * loan_tenure):,}", 'Pre-payment Charges:', 'Nil (floating rate loans)'],
    ]
    kfs_table = Table(kfs_data, colWidths=[1.8*inch, 1.7*inch, 1.8*inch, 1.7*inch])
    ...
    story.append(kfs_table)
    kfs_note_style = ParagraphStyle('KFSNote', parent=styles['Normal'], fontSize=8,
                                    textColor=colors.HexColor('#555555'), spaceAfter=4)
    story.append(Spacer(1, 0.1*inch))
    story.append(Paragraph(
        "Note: APR is indicative and subject to final sanction terms. Processing fee, insurance, and other charges "
        "will be disclosed in the final loan agreement. Borrower has received and acknowledged this KFS prior to "
        "loan execution as required under RBI Digital Lending Guidelines (September 2022).",
        kfs_note_style))
    story.append(Spacer(1, 0.3*inch))
    ...
    grievance_data = [
        ['Nodal Officer:', 'Grievance Redressal Officer (GRO)'],
        ['Email:', 'grievance@lender.in  |  complaints@lender.in'],
        ['Phone:', '1800-XXX-XXXX (Toll Free, Mon–Sat 9AM–6PM)'],
        ['Response Time:', 'Acknowledgement within 2 working days; Resolution within 30 days'],
        ['Escalation:', 'RBI Integrated Ombudsman Scheme — https://cms.rbi.org.in'],
        ['Complaint Register:', 'Available at branch / online borrower portal'],
    ]
    grievance_table = Table(grievance_data, colWidths=[2.2*inch, 4.8*inch])
    ...
    story.append(Paragraph("PENAL CHARGES DISCLOSURE", heading_style))
    penal_data = [
        ['Late Payment Charge:', 'Rs.500 per month or 2% p.a. on overdue amount (whichever is higher)'],
        ['Bounce / ECS Return Charge:', 'Rs.500 per instance (inclusive of bank charges)'],
        ['Pre-payment (Foreclosure):', 'Nil for floating rate loans; 2% on outstanding for fixed rate'],
        ['Part Pre-payment:', 'Nil (minimum Rs.5,000; allowed after 6 EMIs)'],
        ['Duplicate NOC / Statement:', 'Rs.250 per request'],
        ['Penal Interest Note:', 'Penal charges are NOT compounded and will not be capitalised per RBI circular (Aug 2023).'],
    ]
    penal_table = Table(penal_data, colWidths=[2.2*inch, 4.8*inch])
    penal_table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (0, -1), colors.HexColor('#f0f4f8')),
        ('TEXTCOLOR', (0, 0), (-1, -1), colors.black),
        ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
        ('FONTNAME', (0, 0), (0, -1), 'Helvetica-Bold'),
    ...
    story.append(Paragraph(f"Generated on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Credit Risk Assessment System v8.7", footer_style))
