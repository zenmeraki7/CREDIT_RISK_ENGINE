...
def generate_decision_pdf(decision_data, customer_data, affordability_data, reasons):
    ...
    # Title
    # -----------------------------
    elements.append(Paragraph("<b>Credit Decision Summary</b>", styles['Title']))
    elements.append(Spacer(1, 0.3 * inch))
    ...
    # -----------------------------
    app_info = f"""
    <b>Application ID:</b> {decision_data.get('application_id', 'N/A')}<br/>
    <b>Decision Date:</b> {decision_data.get('timestamp', 'N/A')}<br/>
    <b>Decision:</b> {decision_data.get('decision', 'N/A')}<br/>
    <b>Risk Score:</b> {decision_data.get('risk_score', 'N/A')} / 1000<br/>
    <b>PD:</b> {decision_data.get('pd_percentage', 'N/A')}%
    """
    elements.append(Paragraph(app_info, styles['Normal']))
    elements.append(Spacer(1, 0.3 * inch))
    ...
    customer_table_data = [
        ...
        ['Employment Type', customer_data.get('employment_type', 'N/A')],
        ['Bureau Score', customer_data.get('bureau_score', 'N/A')],
        ['Monthly Income', f"₹{customer_data.get('avg_salary_6m', 0):,}"],
    ...
    # -----------------------------
    elements.append(Paragraph("<b>Affordability Assessment</b>", styles['Heading2']))

    affordability_text = f"""
    Monthly Income: ₹{affordability_data.get('monthly_income', 0):,}<br/>
    Total EMI: ₹{affordability_data.get('total_emi', 0):,}<br/>
    FOIR: {affordability_data.get('foir_percentage', 'N/A')}%<br/>
    Net Disposable: ₹{affordability_data.get('net_disposable', 0):,}
    """
    elements.append(Paragraph(affordability_text, styles['Normal']))
    elements.append(Spacer(1, 0.3 * inch))
    ...
    # Reason Codes
    # -----------------------------
    elements.append(Paragraph("<b>Decision Reasons</b>", styles['Heading2']))
    for i, reason in enumerate(reasons, 1):
        elements.append(Paragraph(f"{i}. {reason}", styles['Normal']))
...
def add_download_button(decision_data, customer_data, affordability_data, reasons):
    ...
    st.download_button(
        ...
        data=pdf_buffer,
        file_name=f"credit_decision_{decision_data.get('application_id', 'NA')}.pdf",
        mime="application/pdf",
        use_container_width=True
    )
